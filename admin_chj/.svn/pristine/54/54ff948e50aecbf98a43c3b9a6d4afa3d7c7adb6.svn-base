...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['keywords'])
        {
            $ex_where .= " AND (user_name LIKE '%" . mysql_like_quote($filter['keywords']) ."%' OR true_name LIKE '%" . mysql_like_quote($filter['keywords']) ."%')";
        }
        if ($filter['rank'])
        {
        ...
        $sql = "SELECT user_id, user_name, true_name, email, is_validated, mobile_phone, user_money, frozen_money, rank_points, pay_points, reg_time, last_time ,nickname " . 
				"FROM " . $GLOBALS['ecs']->table('users') . " AS u " .
                "LEFT JOIN " . $GLOBALS['ecs']->table('wechat_user') . " AS w ON w.ect_uid=u.user_id " . $ex_where .
    ...
    foreach ($user_list as $key => $value) {
        $user_list[$key]['reg_time'] = local_date($GLOBALS['_CFG']['time_format'], $value['reg_time']);
    }
