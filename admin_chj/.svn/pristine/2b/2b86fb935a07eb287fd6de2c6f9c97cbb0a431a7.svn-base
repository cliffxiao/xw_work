...
//-- 订单列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'cardlist')
{
    /* 检查权限 */
    admin_priv('card_list');

    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['01_cardorder_list']);
    ...
    $smarty->assign('lang_consignee',   '购券人');
    ...
    $smarty->display('cardorder_list.htm');
}

/*------------------------------------------------------ */
//-- 订单详细列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'cardlist_info')
{
    /* 根据订单id或订单号查询订单信息 */
    ...
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['02_cardorder_list_info']);
    $smarty->assign('action_link', array('href' => 'order.php?act=cardlist', 'text' => $_LANG['01_cardorder_list']));

    $smarty->assign('full_page',        1);
    $smarty->assign('lang',         $_LANG);

    $card_list = cardlist_info($order_id);
    ...
    $smarty->display('cardlist_info.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    /* 检查权限 */
    admin_priv('order_view');
    $order_type = intval($_REQUEST['order_type'])  == 1 ? 1 : 0;
    ...
    /*根据订单类型选择对应模板*/
    $template = $order_type == 1 ? 'cardorder_list.htm' : 'order_list.htm';
    make_json_result($smarty->fetch($template), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
 ...
 * @return void
 */
function order_list($order_type = '0')
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        if (!empty($_REQUEST['order_type']) || !empty($order_type))
        {
            $filter['order_type'] = intval(max($_REQUEST['order_type'],$order_type));
        }
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, w.nickname, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ".
                " LEFT JOIN " .$GLOBALS['ecs']->table('wechat_user'). " AS w ON u.user_id=w.ect_uid ". $where .
 ...
 * @return void
 */
function cardlist_info($order_id)
{
    $result = get_filter();
    if ($result === false)
    {
        /* 过滤信息 */
        $filter['order_id'] = empty($order_id) ? 0 : intval($order_id);
        ...
        /* 查询 */
        $sql = "SELECT vc.card_id, goods_name, market_price, goods_price, end_date, is_act
                FROM " . $GLOBALS['ecs']->table("order_info") . " AS oi " .
                " LEFT JOIN  " . $GLOBALS['ecs']->table("order_goods") . " AS og ON oi.order_id = og.order_id " .
                " LEFT JOIN  " . $GLOBALS['ecs']->table("virtual_card") . " AS vc ON oi.order_sn = vc.order_sn " .
                " WHERE oi.order_id = ".$filter['order_id'] .
    ...
    foreach ($row AS $key => $value)
    {
        $row[$key]['end_date'] = local_date($GLOBALS['_CFG']['time_format'], $value['end_date']);
        if ($value['is_act'] == 1)
        {
            $row[$key]['status_name'] = $GLOBALS['_LANG']['card_status'][1];
        }
        else
        {
            $row[$key]['status_name'] = $GLOBALS['_LANG']['card_status'][0];
        }
    }
