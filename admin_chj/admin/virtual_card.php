...
elseif ($_REQUEST['act'] == 'edit_replenish')
{
	...
	*/
	...
	$card['card_password'] = $card['card_password'];
...
elseif ($_REQUEST['act'] == 'action')
{
	...
	*/
    ...
    $coded_card_password = $_POST['card_password'];
...
elseif ($_REQUEST['act'] == 'batch_confirm')
{
    ...
    foreach ($data as $line)
    {
        ...
        {
        	case '5':
                $rec[$i]['prdtNo'] = trim($row[4]);
            case '4':
                $rec[$i]['openBrh'] = $row[3];
            case '3':
                $rec[$i]['end_date'] = $row[2];
            case '2':
                $rec[$i]['add_date'] = $row[1];
            case '1':
                $rec[$i]['card_sn']  = $row[0];
                break;
            default:
...
elseif ($_REQUEST['act'] == 'batch_insert')
{
    ...
    foreach ($_POST['checked'] as $key)
    {
        ...
        $rec['openBrh']  = $_POST['openBrh'][$key];
		$rec['prdtNo']   = $_POST['prdtNo'][$key];
        $rec['crc32']    = crc32(AUTH_KEY);
        $rec['end_date'] = $_POST['end_date'][$key];
        $rec['goods_id'] = $_POST['goods_id'];
        $rec['add_date'] = $_POST['end_date'][$key];
...
elseif ($_REQUEST['act'] == 'submit_change')
{
    ...
    if (isset($_POST['old_string']) && isset($_POST['new_string']))
    {
        ...
        while ($row = $db->fetchRow($res))
        {
            ...
            $row['card_password'] = $row['card_password'];
            $row['crc32'] = $new_crc32;
