...
class CategoryBaseModel extends BaseModel {
    ...
    function get_categories_tree($cat_id = 0) {
        if ($cat_id > 0) {
            $parent_id = $this->model->table('category')
                              ->field('parent_id')
    ...
    function get_child_tree($tree_id = 0) {
        ...
        $where['parent_id'] = $tree_id;
        $where['is_show'] = 1;
        $count = $this->model->table('category')
                             ->where($where)
                             ->count();

        if ($count || $tree_id == 0) {
            $child_field = 'cat_id as id, cat_name, parent_id, is_show, cat_desc, cat_image';

            $res = $this->model->table('category')
                              ->field($child_field)->where($where)
                              ->select();  

            foreach ($res as $key => $row) {
                if($row['is_show']){
                    if(empty($row['cat_image'])) {
                        $row['cat_image'] = __PUBLIC__ . '/' . C('no_picture');
                    }
                    $row['url'] = build_uri('category/index', array('id' => $row['id']));
                    if (isset($row['id']) != NULL) {
                        $row['cat_id'] = $this->get_child_tree($row['id']);
                    }
