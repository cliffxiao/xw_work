...
class CategoryModel extends BaseModel {
    ...
    function get_cat_info($cat_id) {
        $where['cat_id'] = $cat_id;
        $field = 'cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id';
        return $this->model->table('category')->field($field)
                           ->where($where)->find();
    }
     ...
     *
     * @access  public
     * @param   string      $catid      分类的ID
     */
    function get_category_goods_byid($catid = '') {
        $sql = 'SELECT goods_id, goods_sn, goods_name, goods_number, market_price, shop_price from '.$this->pre . 'goods where cat_id='.$catid.' and is_on_sale=1 and is_delete=0 order by shop_price asc';
        $res = $this->query($sql);
        $idx = 0;
        $goods = array();
        $is_first = 0;
        foreach ($res as $key => $value) {
            $goods[$idx]['id'] = $value['goods_id'];
            $goods[$idx]['goods_sn'] = $value['goods_sn'];
            $goods[$idx]['name'] = $value['goods_name'];
            $goods[$idx]['goods_number'] = $value['goods_number'];
            $goods[$idx]['market_price'] = intval($value['market_price']);
            $goods[$idx]['shop_price'] = intval($value['shop_price']);
            $goods[$idx]['thumb'] = get_image_path($value['goods_id'], $value['goods_thumb'], true);
            $goods[$idx]['goods_img'] = get_image_path($value['goods_id'], $value['goods_img']);
        	if($value['goods_number']>0 && $is_first != 1){
        		$goods[$idx]['is_first'] = 1;
        		$is_first = 1;
        	}else{
        		$goods[$idx]['is_first'] = 0;
        	}
            $idx++;
        }
        return $goods;
