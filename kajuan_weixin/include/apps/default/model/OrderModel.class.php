...
//    }
     ...
     * 生成卡券订单
     */
    function create_order_kj($order){
    	$table = "order_info";
    	$res = $this->do_insert_sql($order,$table);
    	if($res){
    		$order_id = $this->model->query("select max(order_id) as order_id from ".$this->pre."order_info");
     ...
     * 生成卡券订单产品表
     */
    function create_ordergoods_kj($ordergoods){
    	$table = "order_goods";
    	$res = $this->do_insert_sql($ordergoods,$table);
    	return $res;
    }
     ...
     * 更新库存
     */
    function update_goods_number($goodsid,$goods_number){
    	$res = $this->model->query("update ".$this->pre."goods set goods_number=goods_number-$goods_number where goods_id=$goodsid");
    	return $res;
    }
     ...
     * 查询库存数量
     */
    function query_stock_num($goodsid){
    	$table = "goods";
    	$goods_res = $this->model->query("select goods_number from ".$this->pre."$table where goods_id=$goodsid");
    	return $goods_res[0]['goods_number'];
    }
     ...
     * 查询购券成功的订单信息
     */
    function get_orderinfo_success($order_sn){
    	$table = "order_info";
    	$table2 = "order_goods";
    	$order_res = $this->model->query("select g.goods_number as number,g.market_price as price from ".$this->pre."$table as i left join ".$this->pre."$table2 as g on i.order_id=g.order_id where i.order_sn='$order_sn'");
    	return $order_res[0];
    }
     ...
     * 查询激活状态
     */
    function get_active_success($order_sn){
    	$table = "virtual_card";
    	$order_res = $this->model->query("select is_act from ".$this->pre."$table where order_sn='$order_sn'");
    	return $order_res[0]['is_act'];
    }
     ...
     * 公共插入数据库方法
     */
    function do_insert_sql($data,$table){
    	$key_str = "(";
    	$val_str = "(";
    	$array_key = 0;
    	foreach($data as $okey=>$oval){
    		$array_key++;
    		if($array_key<count($data)){
    			$key_str .= $okey.",";
    			$val_str .= "'$oval'".",";
    		}else{
    			$key_str .= $okey;
    	...
    	$key_str .= ")";
    	$val_str .= ")";
    	$sql = "insert into ".$this->pre."$table $key_str values $val_str";
