 ...
 * @return  integer
 */
function hidden($string = '', $left = 0, $right = 0, $hidden = '*') {
    if(empty($string)){
        $str = $string;
    }elseif ($right == 0) {
        $length = mb_strlen($string, 'utf8');
        $str = str_repeat($hidden, $left);
        $str .= mb_substr($string, $left, $length, 'utf8');
    }elseif ($left == 0) {
        $length = mb_strlen($string, 'utf8') - $right;
        $str = mb_substr($string, 0, $length, 'utf8');
        $str .= str_repeat($hidden, $right);
    }else{
        $length = mb_strlen($string, 'utf8') - $left - $right;
        $str = mb_substr($string, 0, $left, 'utf8');
        $str .= str_repeat($hidden, $length);
        $str .= mb_substr($string, -$right, $right, 'utf8');
    }
    return $str;
}