...
class ClipsBaseModel extends BaseModel {
    ...
    public function get_user_default($user_id) { 
        ...
        $row = $this->table('users')
                    ->field("true_name, user_idcard, pay_points, user_money, credit_line, last_login, is_validated, user_attribute, is_gesture, pay_password")
                    ->where($where)->find();
                    
        $info = array();
        $user_name = $_SESSION['user_name'];
        if(Check::regex($user_name, 'mobile')){
            $info['username'] = hidden($user_name, 3, 4);
        }else{
            $info['username'] = stripslashes($user_name);
        }
        ...
        $ifno['credit_line'] = $row['credit_line'];

        $info['user_attribute'] = $row['user_attribute'];
        $info['is_gesture'] = $row['is_gesture'];
        $info['true_name'] = hidden($row['true_name'], 1);
        $info['user_idcard'] = hidden($row['user_idcard'], 1, 1);
        $info['is_pay_password'] = empty($row['pay_password'])?'0':'1';
        ...
        if ($last_time == 0) {
            $_SESSION['last_time'] = $last_time = NOW_TIME;
        }

        $info['last_time'] = date(C('time_format'), $last_time);
        ...
        $condition['user_id'] = $user_id;
        $condition['_string'] = "`add_time` > '" . strtotime('-1 months') . "'";
        $info['order_count'] = $this->table('order_info')
                                    ->where($condition)->count();        

        $condition['_string'] = "shipping_time > '$last_time'" . order_query_sql('shipped');
        $info['shipped_order'] = $this->table('order_info')
     ...
     * @return  array       $info               默认页面所需资料数组
     */
    function get_user_real_info($user_id)
    {
        $where['user_id'] = $user_id;
        $row = $this->table('users')->field('true_name, user_idcard, mobile_phone')
                    ->where($where)->find();
        $row['md5'] = md5($row['true_name'] . $row['user_idcard']);
        return $row;
    }
    ...
    public function get_tags($goods_id = 0, $user_id = 0) {
        ...
        $arr = $this->table('tag')  
                    ->field('tag_id, user_id, tag_words, COUNT(tag_id) AS tag_count')
                    ->where($condition)
                    ->group('tag_words')->select();
        return $arr;
    }
