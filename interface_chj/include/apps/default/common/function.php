...
function set_affiliate() {
    ...
    if (!empty($_GET['u']) && $config['on'] == 1) {
        if (!empty($config['config']['expire'])) {
            ...
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), NOW_TIME + 3600 * $config['config']['expire'] * $c);
        } else {
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), NOW_TIME + 3600 * 24); // 过期时间为 1 天
        }
    }
}
