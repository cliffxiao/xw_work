...
class UsersModel extends BaseModel {
    ...
    function update_user_info() {
        ...
        /* 查询会员信息 */
        $time = date('Y-m-d');
        $sql = 'SELECT u.true_name,u.user_money,u.email, u.is_gesture, u.user_gesture, u.pay_points, u.user_rank, u.rank_points, ' .
        ...
        if ($row = $this->row($sql)) {
            ...
            $_SESSION['is_gesture'] = $row['is_gesture'];
            $_SESSION['user_gesture'] = $row['user_gesture'];
            $_SESSION['true_name'] = $row['true_name'];

            /* 判断是否是特殊等级，可能后台把特殊会员组更改普通会员组 */
            if ($row['user_rank'] > 0) {
                $special_rank = $this->table('user_rank')
                                     ->field('special_rank')
                                     ->where(array('rank_id'=>$row['user_rank']))
                                     ->getOne();

                if ($special_rank === '0' || $special_rank === null) {
                    $this->table()->where(array('user_id'=>$user_id))
                                  ->save(array('user_rank'=>0));
                    $row['user_rank'] = 0;
                }
            }
            /* 取得用户等级和折扣 */
            if ($row['user_rank'] == 0) {
                // 非特殊等级
                // 根据等级积分计算用户等级（注意：不包括特殊等级）
                $map['special_rank'] = 0;
                $map['min_points'] = array("ELT", intval($row['rank_points']));
                $map['max_points'] = array("GT", intval($row['rank_points']));
                $row = $this->table('user_rank')->field('rank_id, discount')
                     ->where($map)->find();
        ...
        /* 更新登录时间，登录次数及登录ip */
        $data['visit_count'] = array("exp", "visit_count+1");
        $data['error_count'] = 0;
        $data['last_ip'] = real_ip();
        $data['last_login'] = NOW_TIME;
        $this->table()->where(array('user_id'=>$user_id))->save($data);
    }
    ...
    function register($username, $password, $email = null, $other = array()) {
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
            ...
            //$user_id = ECTouch::db()->insert_id();
            $condition['user_id'] = $user_id;

            /* 注册送积分 */
            $register_points = C('register_points');
            if (!empty($register_points)) {
                model('ClipsBase')->log_account_change($user_id, 0, 0, C('register_points'), C('register_points'), L('register_points'));
            }
            ...
            if (isset($affiliate['on']) && $affiliate['on'] == 1) {
                ...
                if ($up_uid) {
                    if (!empty($affiliate['config']['level_register_all'])) {
                        if (!empty($affiliate['config']['level_register_up'])) {
                            ...
                            if ($res['rank_points'] + $affiliate['config']['level_register_all'] <= $affiliate['config']['level_register_up']) {
                                model('ClipsBase')->log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], 0, sprintf(L('register_affiliate'), $user_id, $username));
                            }
                        } else {
                    ...
                    //设置推荐人
                    $this->update($condition, array("parent_id"=>$up_uid));
                }
            }

            //定义other合法的变量数组
            $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'is_validated', 'user_attribute');
            if ($other) {
                ...
                if($other){
                    $this->update($condition, $other);
                }
            }
            model('Users')->update_user_info();      // 更新用户信息
            model('Users')->recalculate_price();     // 重新计算购物车中的商品价格recalculate_price
    ...
    function edit_profile($profile) {
        ...
        if (isset($profile['sex'])) {
            $profile['gender'] = intval($profile['sex']);
            unset($profile['sex']);
        }

        if (!empty($profile['email'])) {
            if (!Check::regex($profile['email'], 'email')) {
                ECTouch::err()->add(sprintf(L('email_invalid'), $profile['email']));
                return false;
            }
        }
        if (!empty($profile['birthday'])) {
            $profile['bday'] = $profile['birthday'];
            unset($profile['birthday']);
        }

        if (!ECTouch::user()->edit_user($profile)) {
    ...
    function add_bonus($user_id, $bouns_sn) {
        ...
        if ($row) {
            if ($row['user_id'] == 0) {
                ...
                if (NOW_TIME > $bonus_time['use_end_date']) {
    ...
    function check_login($user_name, $password) {
        $result = Check::rule(
            array(Check::must($user_name), array(L('msg_mobile_blank'), 'code'=>'0001')),
            array(Check::mobile($user_name), array(L('passport_js.mobile_phone_invalid'), 'code'=>'0002')),
            array(Check::must($password), array(L('passport_js.password_empty'), 'code'=>'0003')),
            array(Check::len($password, 6), array(L('passport_js.password_shorter'), 'code'=>'0004')),            
            array($this->check_user_name($user_name), array(L('username_not_exist'), 'code'=>'0005')),
            array(!$this->get_error_count($user_name, 'error_count'), array(L('passport_js.password_error_count'), 'code'=>'0006'))            
        );
        if(is_array($result)){
            ECTouch::err()->add($result);
            return false;
     ...
     * 获取用户登录错误次数
     */    
    protected function get_error_count($user_name, $field, $count = 5)
    {
        $error_count = $this->table()
                            ->field($field)
                            ->where(array('user_name'=>$user_name))
                            ->getOne();
     ...
     * 更新手势密码
     */
    function edit_gesture($is_gesture = 0, $user_gesture = '')
    {
        $condition['user_id'] = I("session.user_id");
        $data['user_gesture'] = $user_gesture;
        $data['is_gesture'] = $is_gesture;
        $data['gesture_count'] = 0;
        return $this->table()->where($condition)->save($data);
    }

    /**
     * 修改支付密码
     */
    function edit_pay_password($pay_password = '')
    {
        $condition['user_id'] = I("session.user_id");
     ...
     * 获取支付密码
     */
    function get_pay_password()
    {
        $condition['user_id'] = I("session.user_id");
        return $this->table()->field('pay_password')
