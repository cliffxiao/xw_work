...
function get_bonus_type() {
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . ECTouch::ecs()->table('bonus_type') .
            ' WHERE send_type = 3';
    $res = ECTouch::db()->query($sql);

    while ($row = ECTouch::db()->fetchRow($res)) {
...
function get_rank_list($is_special = false) {
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . ECTouch::ecs()->table('user_rank');
    if ($is_special) {
...
function get_user_rank($rankid, $where) {
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . ECTouch::ecs()->table('users') . $where .
...
function get_position_list() {
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height ' .
            'FROM ' . ECTouch::ecs()->table('ad_position');
    $res = ECTouch::db()->query($sql);
...
function get_goods_list($filter) {
    ...
    $sql = 'SELECT goods_id, goods_name, shop_price ' .
            'FROM ' . ECTouch::ecs()->table('goods') . ' AS g ' . $where .
            'LIMIT 50';
    $row = ECTouch::db()->getAll($sql);
...
function get_article_list($filter) {
    ...
    $sql = 'SELECT a.article_id, a.title ' .
            'FROM ' . ECTouch::ecs()->table('article') . ' AS a, ' . ECTouch::ecs()->table('article_cat') . ' AS c ' . $where;
    $res = ECTouch::db()->query($sql);
...
function goods_type_list($selected) {
    $sql = 'SELECT cat_id, cat_name FROM ' . ECTouch::ecs()->table('goods_type') . ' WHERE enabled = 1';
    $res = ECTouch::db()->query($sql);
...
function get_pay_ids() {
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' . ECTouch::ecs()->table('payment') . ' WHERE enabled = 1';
    $res = ECTouch::db()->query($sql);
...
function truncate_table($table_name) {
    $sql = 'TRUNCATE TABLE ' . ECTouch::ecs()->table($table_name);

    return ECTouch::db()->query($sql);
}
...
function get_attr_groups($cat_id) {
    $sql = "SELECT attr_group FROM " . ECTouch::ecs()->table('goods_type') . " WHERE cat_id='$cat_id'";
    $grp = str_replace("\r", '', ECTouch::db()->getOne($sql));
...
function set_filter($filter, $sql, $param_str = '') {
    ...
    setcookie('ECSCP[lastfilterfile]', sprintf('%X', crc32($filterfile)), NOW_TIME + 600);
    setcookie('ECSCP[lastfilter]', urlencode(serialize($filter)), NOW_TIME + 600);
    setcookie('ECSCP[lastfiltersql]', base64_encode($sql), NOW_TIME + 600);
}
...
function cat_exists($cat_name, $parent_cat, $exclude = 0) {
    $sql = "SELECT COUNT(*) FROM " . ECTouch::ecs()->table('category') .
            " WHERE parent_id = '$parent_cat' AND cat_name = '$cat_name' AND cat_id<>'$exclude'";
    return (ECTouch::db()->getOne($sql) > 0) ? true : false;
}

function brand_exists($brand_name) {
    $sql = "SELECT COUNT(*) FROM " . ECTouch::ecs()->table('brand') .
 ...
 */
function admin_info() {
    $sql = "SELECT * FROM " . ECTouch::ecs()->table('admin_user') . "
            WHERE user_id = '$_SESSION[admin_id]'
            LIMIT 0, 1";
    $admin_info = ECTouch::db()->getRow($sql);
...
function suppliers_list_info($conditions = '') {
    ...
    $sql = "SELECT suppliers_id, suppliers_name, suppliers_desc
            FROM " . ECTouch::ecs()->table("suppliers") . "
            $where";

    return ECTouch::db()->getAll($sql);
}
...
function get_user_rank_list() {
    $sql = "SELECT * FROM " . ECTouch::ecs()->table('user_rank') .
            " ORDER BY min_points";
...
function get_member_price_list($goods_id) {
    ...
    $sql = "SELECT user_rank, user_price FROM " .
            ECTouch::ecs()->table('member_price') .
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list) {
    ...
    foreach ($id_list AS $key => $id) {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . ECTouch::ecs()->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = ECTouch::db()->getOne($sql);
        if (!empty($result_id)) {
            $sql = "UPDATE " . ECTouch::ecs()->table('goods_attr') . "
            ...
            $sql = "INSERT INTO " . ECTouch::ecs()->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
        ...
        if ($goods_attr_id[$id] == '') {
            $goods_attr_id[$id] = ECTouch::db()->insert_id();
        }
    }
...
function handle_member_price($goods_id, $rank_list, $price_list) {
    ...
    foreach ($rank_list AS $key => $rank) {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . ECTouch::ecs()->table('member_price') .
                " WHERE goods_id = '$goods_id' AND user_rank = '$rank'";
        if (ECTouch::db()->getOne($sql) > 0) {
            /* 如果会员价格是小于0则删除原来价格，不是则更新为新的价格 */
            if ($price < 0) {
                $sql = "DELETE FROM " . ECTouch::ecs()->table('member_price') .
                        " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            } else {
                $sql = "UPDATE " . ECTouch::ecs()->table('member_price') .
            ...
            if ($price == -1) {
                ...
                $sql = "INSERT INTO " . ECTouch::ecs()->table('member_price') . " (goods_id, user_rank, user_price) " .
...
function handle_other_cat($goods_id, $cat_list) {
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . ECTouch::ecs()->table('goods_cat') .
            " WHERE goods_id = '$goods_id'";
    $exist_list = ECTouch::db()->getCol($sql);

    /* 删除不再有的分类 */
    $delete_list = array_diff($exist_list, $cat_list);
    if ($delete_list) {
        $sql = "DELETE FROM " . ECTouch::ecs()->table('goods_cat') .
                " WHERE goods_id = '$goods_id' " .
    ...
    foreach ($add_list AS $cat_id) {
        // 插入记录
        $sql = "INSERT INTO " . ECTouch::ecs()->table('goods_cat') .
                " (goods_id, cat_id) " .
...
function handle_link_goods($goods_id) {
    $sql = "UPDATE " . ECTouch::ecs()->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
...
function handle_group_goods($goods_id) {
    $sql = "UPDATE " . ECTouch::ecs()->table('group_goods') . " SET " .
            " parent_id = '$goods_id' " .
            " WHERE parent_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    ECTouch::db()->query($sql);
...
function handle_goods_article($goods_id) {
    $sql = "UPDATE " . ECTouch::ecs()->table('goods_article') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    ECTouch::db()->query($sql);
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls) {
    ...
    foreach ($image_descs AS $key => $img_desc) {
        ...
        if ($flag) {
            ...
            $sql = "INSERT INTO " . ECTouch::ecs()->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            ECTouch::db()->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && !C('retain_original_img') && !empty($img_original)) {
                ECTouch::db()->query("UPDATE " . ECTouch::ecs()->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $img_original);
            }
...
function update_goods($goods_id, $field, $value) {
    if ($goods_id) {
        ...
        $sql = "UPDATE " . ECTouch::ecs()->table('goods') .
                " SET $field = '$value' , last_update = '" . NOW_TIME . "' " .
...
function delete_goods($goods_id) {
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . ECTouch::ecs()->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id) . " AND is_delete = 1";
    $goods_id = ECTouch::db()->getCol($sql);
    if (empty($goods_id)) {
        return;
    }
    ...
    $sql = "SELECT goods_thumb, goods_img, original_img " .
            "FROM " . ECTouch::ecs()->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    $res = ECTouch::db()->query($sql);
    while ($goods = ECTouch::db()->fetchRow($res)) {
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . ECTouch::ecs()->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . ECTouch::ecs()->table('products') .
            " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);

    /* 删除商品相册的图片文件 */
    $sql = "SELECT img_url, thumb_url, img_original " .
            "FROM " . ECTouch::ecs()->table('goods_gallery') .
            " WHERE goods_id " . db_create_in($goods_id);
    $res = ECTouch::db()->query($sql);
    while ($row = ECTouch::db()->fetchRow($res)) {
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . ECTouch::ecs()->table('goods_gallery') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . ECTouch::ecs()->table('collect_goods') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('goods_article') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('goods_attr') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('goods_cat') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('member_price') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('group_goods') . " WHERE parent_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('group_goods') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('link_goods') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('link_goods') . " WHERE link_goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('tag') . " WHERE goods_id " . db_create_in($goods_id);
    ECTouch::db()->query($sql);
    $sql = "DELETE FROM " . ECTouch::ecs()->table('comment') . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id);
    ECTouch::db()->query($sql);

    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . ECTouch::ecs()->table('virtual_card') . " WHERE goods_id " . db_create_in($goods_id);
    if (!ECTouch::db()->query($sql, 'SILENT') && ECTouch::db()->errno() != 1146) {
        die(ECTouch::db()->error());
    }
...
function check_goods_sn_exist($goods_sn, $goods_id = 0) {
    ...
    if (empty($goods_id)) {
        $sql = "SELECT goods_id FROM " . ECTouch::ecs()->table('goods') . "
...
function get_attr_list($cat_id, $goods_id = 0) {
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price " .
            "FROM " . ECTouch::ecs()->table('attribute') . " AS a " .
            "LEFT JOIN " . ECTouch::ecs()->table('goods_attr') . " AS v " .
...
function get_goods_type_specifications() {
    ...
    $sql = "SELECT DISTINCT cat_id
            FROM " . ECTouch::ecs()->table('attribute') . "
            WHERE attr_type = 1";
    $row = ECTouch::db()->GetAll($sql);
...
function build_attr_html($cat_id, $goods_id = 0) {
    ...
    foreach ($attr AS $key => $val) {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
                L('spec_price') . ' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />' :
...
function get_linked_goods($goods_id) {
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . ECTouch::ecs()->table('link_goods') . " AS lg, " .
            ECTouch::ecs()->table('goods') . " AS g " .
...
function get_group_goods($goods_id) {
    $sql = "SELECT gg.goods_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']') AS goods_name " .
            "FROM " . ECTouch::ecs()->table('group_goods') . " AS gg, " .
            ECTouch::ecs()->table('goods') . " AS g " .
...
function get_goods_articles($goods_id) {
    $sql = "SELECT g.article_id, a.title " .
            "FROM " . ECTouch::ecs()->table('goods_article') . " AS g, " .
            ECTouch::ecs()->table('article') . " AS a " .
...
function goods_list($is_delete, $real_goods = 1, $conditions = '') {
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . ECTouch::ecs()->table('goods') . " AS g WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = ECTouch::db()->getOne($sql);
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
                ...
                " FROM " . ECTouch::ecs()->table('goods') . " AS g WHERE is_delete='$is_delete' $where" .
...
function check_goods_product_exist($goods_id, $conditions = '') {
    ...
    $sql = "SELECT goods_id
            FROM " . ECTouch::ecs()->table('products') . "
            WHERE goods_id = '$goods_id'
            " . $conditions . "
            LIMIT 0, 1";
    $result = ECTouch::db()->getRow($sql);
...
function product_goods_attr_list($goods_id) {
    ...
    $sql = "SELECT goods_attr_id, attr_value FROM " . ECTouch::ecs()->table('goods_attr') . " WHERE goods_id = '$goods_id'";
    $results = ECTouch::db()->getAll($sql);
...
function get_goods_specifications_list($goods_id) {
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . ECTouch::ecs()->table('goods_attr') . " AS g
                LEFT JOIN " . ECTouch::ecs()->table('attribute') . " AS a
...
function product_list($goods_id, $conditions = '') {
    ...
    if ($result === false) {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . ECTouch::ecs()->table('products') . " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = ECTouch::db()->getOne($sql);

        $sql = "SELECT product_id, goods_id, goods_attr, product_sn, product_number
                FROM " . ECTouch::ecs()->table('products') . " AS g
...
function get_product_info($product_id, $filed = '') {
    ...
    $sql = "SELECT $filed FROM  " . ECTouch::ecs()->table('products') . " WHERE product_id = '$product_id'";
    $return_array = ECTouch::db()->getRow($sql);
...
function check_goods_specifications_exist($goods_id) {
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " . ECTouch::ecs()->table('attribute') . " AS a, " . ECTouch::ecs()->table('goods') . " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0) {
    ...
    if (empty($product_id)) {
        $sql = "SELECT product_id FROM " . ECTouch::ecs()->table('products') . "
...
function check_product_sn_exist($product_sn, $product_id = 0) {
    ...
    $sql = "SELECT goods_id FROM " . ECTouch::ecs()->table('goods') . "WHERE goods_sn='$product_sn'";
    if (ECTouch::db()->getOne($sql)) {
        return true;    //重复
    }
...
function reformat_image_name($type, $goods_id, $source_img, $position = '') {
    $rand_name = NOW_TIME . sprintf("%03d", mt_rand(1, 999));
