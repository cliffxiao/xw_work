...
class integrate
{
    ...
    private $field_other = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone');
     ...
     * @return void
     */
    function login($user_name, $password, $remember = null)
    {
        if ($this->check_user($user_name, $password) > 0) {
            if ($this->need_sync) {
                $this->sync($user_name, $password);
            }
            $this->set_session($user_name);
            $this->set_cookie($user_name, $remember);
     ...
     * @return int
     */
    function add_user($user_name, $password, $email = '', $gender = -1, $bday = 0, $reg_date = NOW_TIME, $md5password = '')
    {
        /* 将用户添加到整合方 */
        if ($this->check_user($user_name) > 0) {
    ...
    {
        if (empty($cfg['user_name'])) {
            return false;
        }

        $values = array();
        $values['ec_salt'] = "`ec_salt` = '0'";
        $values['salt'] = "`salt` = '0'";        
        ...
        if ((! empty($cfg['email'])) && $this->field_email != 'NULL') {
            ...
            $sql = "SELECT " . $this->field_id . 
                   ...
                   " AND " . $this->field_name . " != '$cfg[user_name]'";
            ...
            // 检查是否为新E-mail
            if ($this->check_email($cfg['email']) == false) {
                // 新的E-mail
                $sql = "UPDATE " . $this->table() . 
                       " SET is_validated = 0 " . 
                       " WHERE user_name = '$cfg[username]'";
                $this->db->query($sql);
            }
        ...
        /* 修改在其他资料 */
        if (!empty($cfg['other'])) {
            foreach ($cfg['other'] as $key => $value) {
                if (in_array($key, $this->field_other)) {
                    $values[] = "`$key`='" . htmlspecialchars(trim($value)) . "'";
                }
            }
        }
        ...
        if ($values) {
            $sql = "UPDATE " . $this->table() . 
                   " SET " . implode(', ', $values) . 
                   " WHERE " . $this->field_name . "='" . $cfg['user_name'] . 
                   "' LIMIT 1";
            ...
            if ($this->need_sync) {
                if (empty($cfg['md5password'])) {
                    $this->sync($cfg['user_name']);
                } else {
                    $this->sync($cfg['user_name'], '', $cfg['md5password']);
                }
            }
        }
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop)) {
            ...
            if ($col) {
                $sql = "UPDATE " . $this->table() . 
                       " SET parent_id = 0 WHERE " . db_create_in($col, 'parent_id'); // 将删除用户的下级的parent_id 改为0
                $this->db->query($sql);
                $sql = "DELETE FROM " . $this->table() . 
                       " WHERE " . db_create_in($col, 'user_id'); // 删除用户
     ...
     * @return void
     */
    function get_profile_by_name($user_name)
    {
        $sql = "SELECT " . $this->field_id . " AS user_id," . 
               $this->field_name . " AS user_name," . 
               $this->field_email . " AS email," . 
               $this->field_mobile . " AS mobile," . 
               $this->field_gender . " AS sex," . 
               $this->field_bday . " AS birthday," . 
               $this->field_reg_date . " AS reg_time, " . 
               $this->field_passwd_question . " AS passwd_question," . 
               $this->field_pass . " AS password " . 
               " FROM " . $this->table() . 
               " WHERE " . $this->field_name . "='$user_name'";
     ...
     * @return int
     */
    function check_user($user_name, $password = null)
    {        
        /* 如果没有定义密码则只检查用户名 */
        if ($password === null) {
            $sql = "SELECT " . $this->field_id . 
    ...
    {
        if (!empty($email) && $this->field_email != 'NULL') {
            /* 检查email是否重复 */
            $sql = "SELECT " . $this->field_id . 
                   " FROM " . $this->table() . 
     ...
     * @return void
     */
    function set_cookie($user_name = '', $remember = null)
    {
        if (empty($user_name)) {
            /* 摧毁cookie */
            $time = time() - 3600;
            ...
            setcookie("ECS[username]", $user_name, $time, $this->cookie_path, $this->cookie_domain);
            $sql = "SELECT " . $this->field_id . "," . $this->field_pass . 
                   " FROM " . $this->table() .  
                   " WHERE " . $this->field_name . "='$user_name' LIMIT 1";
            $row = $this->db->getRow($sql);
            if ($row) {
     ...
     * @return void
     */
    function table($str = null)
    {
        $str = empty($str) ? $this->user_table : $str;
     ...
     * @return void
     */
    function sync($user_name, $password = '', $md5password = '')
    {
        if ((! empty($password)) && empty($md5password)) {
            $md5password = md5($password);
        }
        
        $main_profile = $this->get_profile_by_name($user_name);
        ...
        if (empty($profile)) {
            ...
            if (empty($md5password)) {
                $sql = "INSERT INTO " . $this->table() . 
                       ...
                       ", " . $this->field_bday .   
                       ", " . $this->field_reg_date . ")" . 
                       " VALUES('$user_name', '" . $main_profile['email'] . "','" . $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "')";
            } else {
                $sql = "INSERT INTO " . $this->table() . 
                       "(" . $this->field_name . 
                       ", " . $this->field_email . 
                       ...
                       ", " . $this->field_reg_date . 
                       ", " . $this->field_pass . ")" .                 
                       " VALUES('$user_name', '" . $main_profile['email'] . "','" . $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "', '$md5password')";
            }
            $this->db->query($sql);
     ...
     * @return void
     */
    function set_points($user_name, $credits)
    {
    ...
    {
        return $this->get_profile_by_name($user_name);
    }
