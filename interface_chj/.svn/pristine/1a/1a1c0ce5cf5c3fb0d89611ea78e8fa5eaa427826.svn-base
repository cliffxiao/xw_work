...
class UserController extends CommonController
{
    ...
    public function __construct()
    {
        ...
        $this->check_login();
        // 用户信息
        //$this->info = model('ClipsBase')->get_user_default($this->user_id);
        // 如果是显示页面，对页面进行相应赋值
        //assign_template();
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if(model('Users')->check_login($username, $password) != true)
            {
                ECTouch::err()->error();
            }
            ...
            if (self::$user->login($username, $password, isset($_POST['remember']))) {
                ...
                */
               	M('users')->where(array('user_name'=>$username))->setInc('error_count');
                $result[] = L('error_password');
               	$result['code'] = '0007';
                ECTouch::err()->add($result);
               	ECTouch::err()->error();
            }
    ...
    public function mobile()
    {
        if(IS_POST)
        {
            ...
            $rules[] = array(Check::regex($mobile, 'require'), array(L('msg_mobile_blank'), 'code'=>'0001'));
            $rules[] = array(Check::regex($mobile, 'mobile'), array(L('passport_js.mobile_phone_invalid'), 'code'=>'0002'));
            ...
            else
            {
                ...
                $array['code'] = '0000';
                $array['count'] = model('Users')->check_user_name($mobile);
                ECTouch::err()->add($array);            
                ECTouch::err()->success('', $back_act);
            }             
        }
        else
        {
            $this->back_act = url('user/register');    
            $this->assign('title', L('register'));
            $this->assign('back_act', $this->back_act);
            $this->display('user_mobile.dwt');            
        }
    }
    ...
    public function register()
    {
        ...
        if (IS_POST) {
            ...
            $rules[] = array(Check::regex($agreement, 'require'), array(L('passport_js.agreement'), 'code'=>'0001'));
            if($enabled_sms == 1)
            {
                $other['mobile_phone'] = $username = I('post.mobile');
                $other['user_attribute'] = 1; // 个人属性
                $mobile_code = I('post.mobile_code');
                //$sms_code = I('post.sms_code');
                //$rules[] = array(Check::same($sms_code, I('session.sms_code')), array(L('sms_code_error'), 'code'=>'0004'));
                $rules[] = array(Check::regex($mobile_code, 'require'), array(L('mobile_code_error'), 'code'=>'0005')); 
                $rules[] = array(Check::same($mobile_code, I('session.sms_mobile_code')), array(L('mobile_code_error'), 'code'=>'0006'));        
            }
            else
            {
                $username = I('post.username'); 
                ...
                if (intval(C('captcha')) > 0) 
                {
                    ...
                    $rules[] = array(Check::regex($captcha, 'require'), array(L('invalid_captcha'), 'code'=>'0002'));
                    $rules[] = array(Check::same(strtoupper($captcha), I('session.ectouch_verify')), array(L('invalid_captcha'), 'code'=>'0003'));
                }
                $rules[] = array(Check::regex($username, 'require'), array(L('passport_js.username_empty'), 'code'=>'0007'));
                $rules[] = array(Check::len($username, 6, 15), array(L('passport_js.username_length'), 'code'=>'0008'));
                $rules[] = array(Check::regex($email, 'require'), array(L('passport_js.email_empty'), 'code'=>'00009'));
                $rules[] = array(Check::regex($email, 'email'), array(('passport_js.email_invalid'), 'code'=>'00010'));              
            }
            $rules[] = array(Check::regex($password, 'require'), array(L('passport_js.password_empty'), 'code'=>'0012'));
            $rules[] = array(Check::len($password, 6, 20), array(L('passport_js.password_length'), 'code'=>'0013'));
            $rules[] = array(!Check::regex($password, 'number'), array(L('passport_js.password_rule'), 'code'=>'0014'));  
            $rules[] = array(!Check::regex($password, 'english'), array(L('passport_js.password_rule'), 'code'=>'0014'));
            $rules[] = array(!Check::regex($password, 'symbol'), array(L('passport_js.password_rule'), 'code'=>'0014'));
            $rules[] = array(!strpos($password, ' '), array(L('passwd_balnk'), 'code'=>'0015'));
            ...
            if (model('Users')->register($username, $password, $email, $other) !== false) {
                // 判断是否需要自动发送注册邮件
                if (C('member_email_validate') && C('send_verify_email') && $email) {
                    model('Users')->send_regiter_hash($_SESSION['user_id']);
                }
                $ucdata = empty(self::$user->ucdata) ? "" : self::$user->ucdata;
                $array[] = sprintf(L('register_success'), $username . $ucdata);
                $array['code'] = '0000';
                ...
                ECTouch::err()->error(L('sign_up'), url('register'));
            }
            exit();
        }
        ...
        if (intval(C('sms_signin')) > 0) {
            ...
            // 随机code
            //$this->assign('sms_code', $this->sms_code());
        }
    ...
    public function get_password_phone()
    {
        ...
        if (intval(C('sms_signin')) > 0) {
            ...
            if (IS_POST) {
                ...
                //$rules[] = array(Check::same($sms_code, I('session.sms_code')), array(L('sms_code_error'), 'code'=>'0001'));
                $rules[] = array(Check::regex($mobile_code, 'require'), array(L('mobile_code_error'), 'code'=>'0002')); 
                $rules[] = array(Check::same($mobile_code, I('session.sms_mobile_code')), array(L('mobile_code_error'), 'code'=>'0003'));  

                $rules[] = array(Check::regex($password, 'require'), array(L('passport_js.password_empty'), 'code'=>'0004'));
                $rules[] = array(Check::len($password, 6, 20), array(L('passport_js.password_length'), 'code'=>'0005'));
                $rules[] = array(!Check::regex($password, 'number'), array(L('passport_js.password_rule'), 'code'=>'0006'));  
                $rules[] = array(!Check::regex($password, 'english'), array(L('passport_js.password_rule'), 'code'=>'0006'));
                $rules[] = array(!Check::regex($password, 'symbol'), array(L('passport_js.password_rule'), 'code'=>'0006'));
                $rules[] = array(!strpos($password, ' '), array(L('passwd_balnk'), 'code'=>'0007'));
                ...
                $cfg['user_name'] = $mobile;
                $cfg['password'] = $password;
                if(self::$user->edit_user($cfg)){
                    self::$user->logout();
                    M('users')->where(array('user_name'=>$mobile))->setField('error_count', 0);
                    $array[] = L('edit_password_success');
                    $array['code'] = '0000';
                    ECTouch::err()->add($array);
                    ECTouch::err()->success();
                } else {
                    $array[] = L('edit_password_failure');
                    $array['code'] = '0008';
                    ECTouch::err()->add($array);
                    ECTouch::err()->error();                    
                }
    ...
    public function edit_password()
    {
        ...
        if (IS_POST) {
            ...
            $rules[] = array(Check::regex($old_password, 'require'), array(L('profile_js.old_password_empty'), 'code'=>'0001'));
            $rules[] = array(self::$user->check_user($_SESSION['user_name'], $old_password), array(L('oldpassword_error'), 'code'=>'0002'));   
              
            $rules[] = array(Check::regex($new_password, 'require'), array(L('profile_js.new_password_empty'), 'code'=>'0003'));            
            $rules[] = array(Check::len($new_password, 6, 20), array(L('passport_js.password_length'), 'code'=>'004'));
            $rules[] = array(!Check::regex($new_password, 'number'), array(L('passport_js.password_rule'), 'code'=>'0005'));  
            $rules[] = array(!Check::regex($new_password, 'english'), array(L('passport_js.password_rule'), 'code'=>'0005'));
            $rules[] = array(!Check::regex($new_password, 'symbol'), array(L('passport_js.password_rule'), 'code'=>'0005'));
            $rules[] = array(!strpos($new_password, ' '), array(L('passwd_balnk'), 'code'=>'0006'));
            $rules[] = array(Check::same($new_password, $re_password), array(L('profile_js.both_password_error'), 'code'=>'0007'));
            $result = Check::rule($rules);
            if(is_array($result))
            {
                ECTouch::err()->add($result);
            ...
            if ((! empty($mobile) && $user_info['mobile'] == $mobile) || ($user_info && (! empty($code) && md5($user_info['user_id'] . C('hash_code') . $user_info['reg_time']) == $code)) || (! empty($question) && $user_info['passwd_question'] == $question) || ($_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id)) {
                ...
                if (self::$user->edit_user($cfg)) {
                    ...
                    $array['code'] = '0000';
                    ECTouch::err()->add($array);
                    ECTouch::err()->success(L('relogin_lnk'));
                } else {
                    $array[] = L('edit_password_failure');
                    $array['code'] = '0008';
                    ECTouch::err()->add($array);
                    ECTouch::err()->success(L('back_page_up'));
                }
            } else {
                    $array[] = L('edit_password_failure');
                    $array['code'] = '0008';
    ...
    public function logout()
    {
        ...
        $result[] = L('logout') . $ucdata;
        $result['code'] = '0000';
        ECTouch::err()->add($result);
        ECTouch::err()->success(L('back_home_lnk'));
        /*
        show_message(L('logout') L('back_up_page'). $ucdata, array(
            L('back_up_page'),
            L('back_home_lnk')
        ), array(
            $this->back_act,
    ...
    public function gesture()
    {
        ...
        if($is_gesture == 1 && I('session.is_gesture') == -1)
        {
            $result[] = L('add_gesture');
        }
        elseif($is_gesture == 1 && I('session.is_gesture') == 1)
        {
            $result[] = L('edit_gesture');
        }
        elseif($is_gesture == 0 && I('session.is_gesture') == 1)
        {
            $result[] = L('close_gesture');
            $user_gesture = '';
        }
    ...
    public function pay_password()
    {
        ...
        {
            $mobile_code = I('post.mobile_code');
            $rules[] = array(Check::regex($mobile_code, 'require'), array(L('mobile_code_error'), 'code'=>'0001')); 
            $rules[] = array(Check::same($mobile_code, I('session.sms_mobile_code')), array(L('mobile_code_error'), 'code'=>'0001')); 
            $result = Check::rule($rules);
            if(is_array($result))
            {
                ECTouch::err()->add($result);
    ...
    public function check_pay_password()
    {
        ...
        if($pay_pwd == $pay_password){
            $result[] = L('pay_password_success');
            $result['code'] = '0000';
            ECTouch::err()->add($result);       
            ECTouch::err()->success();
        }else{
            $result[] = L('pay_password_error');
            $result['code'] = '0001';
            ECTouch::err()->add($result);            
            ECTouch::err()->error();
        }        
    ...
    {
        $password = I('post.password');
        $rules[] = array(Check::regex($password, 'require'), array(L('passport_js.password_empty'), 'code'=>'0001'));
        $rules[] = array(self::$user->check_user($_SESSION['user_name'], $password), array(L('edit_password_failure'), 'code'=>'0002'));
        $result = Check::rule($rules);
        if(is_array($result))
        {
            ECTouch::err()->add($result);
            ...
            ECTouch::err()->add(array('code'=>'0000'));
            ECTouch::err()->success();            
        }     
    }
    ...
    function check_user_real_info()
    {
        ...
        $rules[] = array(Check::regex($true_name, 'require'), array(L('true_name_emtpy'), 'code'=>'0001'));
        $rules[] = array(Check::regex($user_idcard, 'require'), array(L('user_idcard_emtpy'), 'code'=>'0002'));
        $result = Check::rule($rules);
        if(is_array($result))
        {
            ECTouch::err()->add($result);
        ...
        $row = model('ClipsBase')->get_user_real_info($this->user_id);
    ...
    public function  account_log(){
        ...
        //获取剩余余额
        $result['surplus_amount'] = model('ClipsBase')->get_user_surplus($this->user_id);
        if (empty($surplus_amount))
        {
            $surplus_amount = 0;
        }
        //获取余额记录
        $result['account_log'] = model('ClipsBase')->get_account_log($this->user_id, $size, ($page-1)*$size);
